...
describe("CrawlQueue", () => {
	...
	test("scheduleRetry adds item back to queue after delay", async () => {
		...
		const item = { url: "https://example.com/retry", domain: "example.com", depth: 0, retries: 1 };
	...
	test("clearRetries cancels pending retry timers", () => {
		...
		queue.scheduleRetry(
			{ url: "https://example.com/1", domain: "example.com", depth: 0, retries: 1 },
			1000,
		);
		queue.scheduleRetry(
			{ url: "https://example.com/2", domain: "example.com", depth: 0, retries: 1 },
			1000,
		);
